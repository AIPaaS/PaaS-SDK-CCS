...
public class ICCSComponentTest {
    ...
    private int userPwd = UUIDTool.getId();
    private String adminName = "admin";
    private String adminPwd = CiperUtil.decrypt(ConfigConstant.OPERATORS, "ec4c9e0e78f76a69");
    ...
    public void setUp() throws Exception {
        ...
        client.createNode(ConfigSDKUtil.appendUserReadOnlyPathPath(userName),
        ...
        client.createNode(ConfigSDKUtil.appendUserWritablePathPath(userName),
                ConfigSDKUtil.createWritableACL(userName, String.valueOf(userPwd), adminName, adminPwd), writableValue,
                CreateMode.PERSISTENT);
        assertTrue(client.exists(ConfigSDKUtil.appendUserWritablePathPath(userName)));

        client.createNode(ConfigSDKUtil.appendUserReadOnlyPathPath(userName) + readOnlyPath,
                ConfigSDKUtil.createReadOnlyACL(userName, String.valueOf(userPwd), adminName, adminPwd), readonlyValue,
                CreateMode.PERSISTENT);
        assertTrue(client.exists(ConfigSDKUtil.appendUserReadOnlyPathPath(userName) + readOnlyPath));

        client.createNode(ConfigSDKUtil.appendUserReadOnlyPathPath(userName) + readOnlyPathA,
                ConfigSDKUtil.createReadOnlyACL(userName, String.valueOf(userPwd), adminName, adminPwd), readonlyValue,
                CreateMode.PERSISTENT);
    ...
    public void clearData() throws Exception {
        ZKClient client = ZKFactory.getZkClient(configAddr, adminName, adminPwd);
        client.deleteNode(ConfigConstant.UserNodePrefix.FOR_PAAS_PLATFORM_PREFIX + ConfigConstant.SEPARATOR + userName);
        assertFalse(client
                .exists(ConfigConstant.UserNodePrefix.FOR_PAAS_PLATFORM_PREFIX + ConfigConstant.SEPARATOR + userName));
    }
