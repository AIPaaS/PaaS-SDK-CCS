...
public class ConfigFactory {
    ...
    private static transient final Logger logger = LoggerFactory.getLogger(ConfigFactory.class);
    ...
    public static IConfigClient getConfigClient(AuthDescriptor ad) throws Exception {
        ...
        if (configClientCache.containsKey(ad.getPid() + "_" + ad.getServiceId())) {
            configClient = configClientCache.get(ad.getPid() + "_" + ad.getServiceId());
            return configClient;
        }
        ...
        ZKClient client = ZKPoolFactory.getZKPool(result.getConfigAddr(), result.getConfigUser(),
                CiperUtil.decrypt(ConfigCenterConstants.operators, result.getConfigPasswd()), ad.getServiceId()).
        ...
        configClient = new ConfigClient.ConfigClientBuilder(zkAddr,zkUser,CiperUtil.decrypt(ConfigCenterConstants.operators, zkPasswd))
                .serviceId(ad.getServiceId()).timeOut(timeout).build();

        //configClient = new ConfigClientImpl(zkAddr, zkUser, CiperUtil.decrypt(ConfigCenterConstants.operators, zkPasswd), ad.getServiceId(), timeout);
    ...
    private static String apendUserNodePath(AuthDescriptor ad, AuthResult result) {
        return ConfigCenterConstants.UserNodePrefix.FOR_PAAS_PLATFORM_PREFIX +
                PaaSConstant.UNIX_SEPERATOR + result.getUserId() +
